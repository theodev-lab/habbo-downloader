...
async function getPosters() {
	...
	const regex = /(poster_\d+)_(?:name|desc)=/gim
...
async function parse(json) {
	...
	const all = [...json.roomitemtypes.furnitype, ...json.wallitemtypes.furnitype]
	...
	all.forEach((item) => {
		map.push({ revision: item.revision, name: `${item.classname.replace('*', '_')}_icon.png` }, { revision: item.revision, name: `${item.classname.split('*')[0]}.swf` })
	})
	...
	await fetchMany(
		[...all].map((item) => {
			return {
				...
				dst: config.revision ? `dcr/hof_furni/${item.revision}/${item.name}` : `dcr/hof_furni/${item.name}`
			}
		})
	)
}
