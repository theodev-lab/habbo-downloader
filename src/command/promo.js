...
const regex = /web_promo_small\/([^;]+)/gim

async function parse(txt) {
	const match = [...txt.matchAll(regex)]

	return new Set(match.map((match) => match[1].trim()))
}

async function collectText() {
	const domain = ['com.br', 'com.tr', 'com', 'de', 'es', 'fi', 'fr', 'it', 'nl']

	const all = await Promise.allSettled(domain.map((d) => fetchText(`https://www.habbo.${d}/gamedata/external_variables/0`)))
