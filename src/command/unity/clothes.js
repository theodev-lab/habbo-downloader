...
async function parse(txt) {
	...
	all.map.lib.forEach((item) => {
		...
		map.push({ revision: item['@_revision'], name: name }, { revision: item['@_revision'], name: `${name}.manifest` })
	})
	...
	await fetchMany(
		[...all].map((item) => {
			return {
				...
				dst: config.revision ? `unity/clothes/${item.revision}/${item.name}` : `unity/clothes/${item.name}`
			}
		})
	)
}
