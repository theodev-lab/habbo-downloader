...
async function parse(json) {
	...
	all.forEach((item) => {
		...
		map.push({ revision: item.revision, name: name }, { revision: item.revision, name: `${name}.manifest` })
	})
	...
	await fetchMany(
		[...all].map((item) => {
			return {
				...
				dst: config.revision ? `unity/furnitures/${item.revision}/${item.name}` : `unity/furnitures/${item.name}`
			}
		})
	)
}
