...
const logger = new Logger({ level: argv.verbose ? 'debug' : 'info' })
...
async function runCommand (command, isUnity) {
  ...
  try {
    ...
    context[level](
      'finished downloaded=%d skipped=%d failed=%d duration=%s',
      stats.downloaded,
      stats.skipped,
      stats.failed,
      duration
    ...
    return { command, failed: false, stats, duration }
  } catch (err) {
    const stats = stopStats()
    const duration = formatDuration(Date.now() - startedAt)

    context.error(
      'aborted downloaded=%d skipped=%d failed=%d duration=%s',
      stats.downloaded,
      stats.skipped,
      stats.failed,
      duration,
    ...
    return { command, failed: true, stats, duration, error: err }
  }
}

async function writeErrorLog (results, totals, startedAt) {
  ...
  const fileLogger = new Logger({
    colorize: false,
    stdout: { write: (line) => lines.push(line.trimEnd()) },
    stderr: { write: (line) => lines.push(line.trimEnd()) },
  })
  ...
  results.forEach((result) => {
    ...
    result.stats.errors.forEach((err) => {
      fileLogger.child(result.command).error(
        'download failed %s %s',
        err.status || '-',
        err.src
      )
...
async function printSummary (results, startedAt) {
  ...
  const duration = formatDuration(Date.now() - startedAt)
  await writeErrorLog(results, totals, startedAt)
